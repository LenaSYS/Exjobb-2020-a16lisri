...
$OutputtoTextfile = fopen("query2_mysql.csv", 'a') or die("Unable to open file!");
...
for($i = 1; $i <=1000; $i++) 
{
    // Starting clock time in seconds - start inside loop to monitor transaction instance not the overall transaction time
        ...
        $query="select ID,sum(Data_value) as total from business where Series_title_1 = 'Glass and glass products' group by ID having total > 1000";
        $mysqli->query("$query");
