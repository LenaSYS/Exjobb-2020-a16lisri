...
$OutputtoTextfile = fopen("query4_mysql.csv", 'a') or die("Unable to open file!");
...
for($i = 1; $i <=1000; $i++)
{
    // Starting clock time in seconds - start inside loop to monitor transaction instance not the overall transaction time
        ...
        $query="select ID, sum(period) as total from business group by ID order by total LIMIT 100";
        $mysqli->query("$query");
