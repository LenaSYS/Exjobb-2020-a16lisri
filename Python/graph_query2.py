...
def readCSVdata():
    with open('Datainsamling/query2_mysql.csv', 'r') as f:
        reader = csv.reader(f)
        for row in reader:
            every_other = row[1::2]
    ...
    with open('Datainsamling/query2_mongodb.csv', 'r') as f:
        reader = csv.reader(f)
        for row in reader:
            every_other = row[1::2]
            for i in every_other:
...
def render():
    ...
    ax2.set_title('Query 2')
    ax2.set_xlabel('Mean and standard deviation')
    ax2.set_ylabel('Loading times (ms)')

    #Rita ut linjediagram
    ax.plot(datalist, color="blue", alpha = 1, label = "MySQL (Query 2)")
    ax.plot(datalist2, color="red", alpha= 1, label = "MongoDB (Query 2)")
    ...
    #Skapa stapeldiagram
    ax2.bar(0, mean1, color="blue", label = "MySQL (Query 2)", width= 0.5, align='center', alpha= 0.8, yerr= sd1, capsize=10)
    ax2.bar(1, mean2, color="red", label = "MongoDB (Query 2)", width= 0.5, align='center', alpha= 0.8, yerr=sd2, capsize=10)
